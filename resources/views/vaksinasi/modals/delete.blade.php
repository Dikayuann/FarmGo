    ...
    <div class="flex min-h-full items-center justify-center p-4">
        ...
        <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div class="mt-2" x-show="currentRecord">
                            <p class="text-sm text-gray-500">
                                Apakah Anda yakin ingin menghapus data vaksinasi
                                <span class="font-semibold text-gray-700" x-text="currentRecord?.jenis_vaksin"></span>
                                untuk hewan
                                <span class="font-semibold text-gray-700"
                                    x-text="currentRecord?.animal?.nama_hewan"></span>
                                pada tanggal
                                <span class="font-semibold text-gray-700"
                                    x-text="currentRecord ? new Date(currentRecord.tanggal_vaksin).toLocaleDateString('id-ID') : ''"></span>?
            ...
            <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse gap-3">
                <form x-show="currentRecord" :action="currentRecord ? `{{ url('vaksinasi') }}/${currentRecord.id}` : ''"
                    method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
